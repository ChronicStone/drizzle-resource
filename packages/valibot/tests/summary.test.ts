...
describe("automatic summary response schemas", () => {
  it("infers aggregate validators and their output without repeating the summary", () => {
    ...
    type Output = v.InferOutput<typeof validator>;
    ...
    expect(v.parse(validator, populated)).toEqual(populated);
    for (const invalid of [
      ...
      expect(() =>
        v.parse(validator, { ...payload, summary: { ...emptySummary, ...invalid } }),
  ...
  it("supports default rows and keeps unrequested summaries out of the result type", () => {
    ...
    expect(v.parse(validator, { rows: [], pageInfo, summary: emptySummary })).toEqual({
      rows: [],
      pageInfo,
      summary: emptySummary,
    });
    const without = responseSchema(resource, { view: "list", summary: false });
    type Without = v.InferOutput<typeof without>;
    expectTypeOf<Without>().not.toHaveProperty("summary");
    expect(v.parse(without, { rows: [], pageInfo })).toEqual({ rows: [], pageInfo });
  ...
  it("requires an explicit override for opaque SQL instead of guessing its runtime type", () => {
    ...
    const validator = responseSchema(custom, { summary: v.object({ value: v.number() }) });
    expect(v.parse(validator, { rows: [], pageInfo, summary: { value: 0 } })).toEqual({
      rows: [],
      pageInfo,
      summary: { value: 0 },
    });
