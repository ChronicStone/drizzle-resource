...
## Trusted server executions

Background work may need larger pages than an interactive endpoint. Set an execution limit on the direct resource call instead of widening the resource's public validation policy:

```ts twoslash
import { ordersResource, request } from "./twoslash-support";

await ordersResource.queryIds({
  request: {
    ...request,
    pagination: { mode: "cursor", cursor: null, pageSize: 5_000 },
  },
  context: { orgId: "acme" },
  execution: { maxPageSize: 5_000 },
});
```

`execution` is a server-side method option. It is absent from `QueryRequestInput` and every generated transport schema, so clients cannot request the larger limit. Keep the value application-owned and never derive it from request data. Field policy, authorization scope, filter limits, sorting rules, and cursor validation remain unchanged.

## Response schemas

`responseSchema` starts with Drizzle's `createSelectSchema` for every selected table, then follows the query hydration default or an explicit named profile. A `one` relation is nullable and a `many` relation is an array, matching Drizzle's hydrated output.
...
const ordersResponse = responseSchema(ordersResource, {
  ...
  override: {
    columns: {
      totalAmount: (schema) => schema.transform((value) => value.toFixed(2)),
    },
    relations: {
      customer: {
        columns: {
          name: (schema) => schema.transform((value) => value.trim()),
        },
      },
    },
  },
...
The selected profile changes the response row type and schema. The override changes validation and output parsing only; it does not modify selected rows, authorization, or SQL.
