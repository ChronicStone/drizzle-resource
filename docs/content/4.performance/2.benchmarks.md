...
These are real measurements from a benchmark suite run against an orders/HR-style dataset with ~50k rows, multiple relations, and several facet types. The benchmarks cover four progressive optimization stages.
...
Counterintuitively, most facet scenarios got *worse* after indexing:
